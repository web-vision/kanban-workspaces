...
export class ModalController {
  ...
  currentStageFormContext: any = null;
  currentStageFormData: any = null;
  ...
  setupModalEvents() {
    ...
    tabBtns.forEach((btn) => {
      btn.addEventListener("click", (e) => {
        this.switchModalTab((e.target as HTMLElement).dataset.tab)
      })
    })
  ...
  handleAddComment() {
    const commentTextarea = document.getElementById("newComment") as HTMLTextAreaElement | null
    const addCommentBtn = document.getElementById("addComment") as HTMLButtonElement | null
  ...
  openSendToStageModal(formData, context) {
    ...
    const commentsInput = document.getElementById("stageComments") as HTMLInputElement | null
    const additionalRecipientsInput = document.getElementById("stageAdditionalRecipients") as HTMLInputElement | null
  ...
  async handleSendToStageSubmit() {
    ...
    const submitBtn = document.getElementById("submitSendToStageBtn") as HTMLButtonElement | null

    // Collect form data
    const comments = (document.getElementById("stageComments") as HTMLInputElement | null)?.value || ""
    const additional = (document.getElementById("stageAdditionalRecipients") as HTMLInputElement | null)?.value || ""

    // Get checked recipients
    const recipients = []
    document.querySelectorAll(".t3js-workspace-recipient:checked").forEach((cb: HTMLInputElement) => {
